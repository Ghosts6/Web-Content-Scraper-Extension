...
beforeEach(() => {
  ...
  document.body.innerHTML = `
    ...
    <img src="data:image/png;base64,abc123==" alt="" />
...
describe('extractPageContent', () => {
  ...
  describe('Clean Mode', () => {
    ...
    test('works with cleanMode false', () => {
...
describe('extractWithSelectors', () => {
  test('extracts content using CSS selectors', () => {
    const result = extractWithSelectors({ title: '.article-title' });
    expect(result.title).toBe('Article Title');
  });

  test('returns single string for single match', () => {
    const result = extractWithSelectors({ title: 'h1.article-title' });
    expect(result.title).toBe('Article Title');
    expect(typeof result.title).toBe('string');
  });

  test('returns array for multiple matches', () => {
    const result = extractWithSelectors({ headings: 'h1, h2' });
    expect(Array.isArray(result.headings)).toBe(true);
    expect(result.headings).toContain('Main Heading');
  });

  test('returns empty array for non-matching selectors', () => {
  ...
  test('handles empty selector string', () => {
    // Empty string fails querySelector — warn is expected, suppress it
    const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});
    const result = extractWithSelectors({
      empty: '',
      valid: 'h1:not(.article-title)',
    });
    expect(result.empty).toBeUndefined();
    expect(result.valid).toBe('Main Heading');
    expect(warnSpy).toHaveBeenCalledWith('[Scraper] Invalid selector skipped: ""');
    warnSpy.mockRestore();
  });

  test('handles complex nested selectors', () => {
    const result = extractWithSelectors({
  ...
  test('handles :nth-child selectors', () => {
    ...
    `;
    const result = extractWithSelectors({ secondItem: 'li:nth-child(2)' });
    expect(result.secondItem).toBe('Second item');
  });

  test('handles ID selectors with special characters', () => {
    document.body.innerHTML += '<div id="special-id">Special Content</div>';
    const result = extractWithSelectors({ special: '#special-id' });
    expect(result.special).toBe('Special Content');
  });

  test('handles elements with only whitespace', () => {
    document.body.innerHTML += '<p class="ws-only">   </p>';
    const result = extractWithSelectors({ whitespace: 'p.ws-only' });
    expect(result.whitespace).toEqual([]);
  });

  test('handles elements with no text', () => {
    document.body.innerHTML += '<div data-empty></div>';
    const result = extractWithSelectors({ empty: 'div[data-empty]' });
    expect(result.empty).toEqual([]);
  });

  describe('Clean Mode with Selectors', () => {
    test('excludes content inside noise elements', () => {
      document.body.innerHTML += '<nav><p class="nav-p">Nav content</p></nav>';
      const result = extractWithSelectors({ navContent: 'nav p' }, true);
      expect(result.navContent).toEqual([]);
    });

    test('preserves content outside noise elements', () => {
      const result = extractWithSelectors({ title: 'h1.article-title' }, true);
      expect(result.title).toBe('Article Title');
    });
  });
});